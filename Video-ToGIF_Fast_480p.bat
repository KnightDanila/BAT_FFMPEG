...
REM //////////////////// MAIN \\\\\\\\\\\\\\\\\\\\\\\\\
		...
		ffmpeg.exe -i %%x -vf fps=10,scale=-1:480:flags=lanczos,palettegen Video-ToGIF_palette.png -y
		ffmpeg.exe -i %%x -i Video-ToGIF_palette.png -filter_complex "fps=15,scale=-1:480:flags=lanczos[x];[x][1:v]paletteuse=dither=bayer:bayer_scale=5" "%RESULTS_DIR%\%%~nx_output_Fast_480p.gif"
