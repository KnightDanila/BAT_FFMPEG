...
REM //////////////////// MAIN \\\\\\\\\\\\\\\\\\\\\\\\\
		...
		REM scale=-1:-1 and scale=-1:360 = dispersion
		ffmpeg.exe -i %%x -vf fps=10,scale=-1:360:flags=lanczos,palettegen Video-ToGIF_palette.png -y
		ffmpeg.exe -i %%x -i Video-ToGIF_palette.png -filter_complex "fps=15,scale=-1:360:flags=lanczos[x];[x][1:v]paletteuse=dither=bayer:bayer_scale=5" "%RESULTS_DIR%\%%~nx_output_Fast_360p.gif"
