...
REM //////////////////// MAIN \\\\\\\\\\\\\\\\\\\\\\\\\
		...
		ffmpeg.exe -i %%x -vf fps=30,scale=-1:480:flags=lanczos,palettegen=max_colors=16 Video-ToGIF_palette.png -y
		ffmpeg.exe -i %%x -i Video-ToGIF_palette.png -filter_complex "fps=24,scale=-1:480:flags=lanczos[x];[x][1:v]paletteuse=dither=bayer:bayer_scale=5" "%RESULTS_DIR%\%%~nx_output_16Bit_480p.gif"
