...
REM //////////////////// MAIN \\\\\\\\\\\\\\\\\\\\\\\\\
		...
		ffmpeg.exe -i %%x -i Video-ToGIF_palette.png -filter_complex "fps=15,scale=-1:240:flags=lanczos[x];[x][1:v]paletteuse=dither=bayer:bayer_scale=5" "%RESULTS_DIR%\%%~nx_Output_32ColorsStyle_240p.gif"
